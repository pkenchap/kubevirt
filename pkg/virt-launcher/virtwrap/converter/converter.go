...
func Convert_v1_VirtualMachineInstance_To_api_Domain(vmi *v1.VirtualMachineInstance, domain *api.Domain, c *convertertypes.ConverterContext) (err error) {
	...
	// TODO: CreateDomainInterfaces function needs to be implemented for PPC64LE
	...
	if ignitiondata != "" && strings.Contains(ignitiondata, "ignition") {
		compute.InitializeQEMUCmdAndQEMUArg(domain)
		...
		domain.Spec.QEMUCmd.QEMUArg = append(domain.Spec.QEMUCmd.QEMUArg, api.Arg{Value: "-machine"}, api.Arg{Value: fmt.Sprintf("pseries-rhel10.0.0,cap-large-decr=off")})
	}
